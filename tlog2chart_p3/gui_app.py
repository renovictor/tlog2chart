...
class Tlog2ChartP2App(tk.Tk):
    ...
    def _on_tool_mode_changed(self):
        ...
        # Leaving ruler mode should reset its unfinished state (but keep drawings until cleared)
        if mode != "RULER":
    ...
    def _build_plot_area(self):
        ...
        ttk.Radiobutton(tool_bar, text="Ruler", value="RULER",
                        variable=self.tool_mode_var,
                        command=self._on_tool_mode_changed).pack(side=tk.LEFT, padx=(8, 0))
        ...
        ttk.Button(tool_bar, text="Clear Markups", command=self._clear_markups).pack(side=tk.LEFT, padx=(12, 0))
    ...
    def _ruler_click(self, event):
        """Handle raw-click ruler: click A then click B, show dx/dy on the clicked axes."""
        ax = event.inaxes
        x = float(event.xdata)
        y = float(event.ydata)

        # If first point is not set, set point A
        if self._ruler_ptA is None or self._ruler_ax is None:
            self._ruler_ax = ax
            self._ruler_ptA = (x, y)

            # remove old ruler drawings if any (optional behavior)
            # keep them until user clears? -> comment out if you want multiple rulers.
            # Clear previous ruler drawings, but do NOT reset the new point-A state
            ...
            # mark point A by placing a small text (optional)
            # We'll not add extra marker to keep it minimal.
            return

        # If user clicked a different axes/subplot, restart A on that axes
        if ax is not self._ruler_ax:
            self._ruler_ax = ax
            self._ruler_ptA = (x, y)
            # do not clear drawings here; user can press Clear Markups
            return

        # Now we have point A and point B on same axes
        x0, y0 = self._ruler_ptA
        x1, y1 = x, y
        ...
        if self._ruler_line is None:
            (self._ruler_line,) = ax.plot([x0, x1], [y0, y1], color="red", linewidth=1.2)
        else:
            self._ruler_line.set_data([x0, x1], [y0, y1])

        # Compose label
        # Use Δ notation; keep it simple
        label = f"dx = {dx:.3f}\ndy = {dy:.3f}"

        # Place text near point B (offset in display coords)
        if self._ruler_text is None:
            self._ruler_text = ax.annotate(
                label,
                xy=(x1, y1),
                xytext=(12, 12),
                textcoords="offset points",
            ...
            self._ruler_text.set_text(label)
            self._ruler_text.xy = (x1, y1)
            self._ruler_text.set_visible(True)

        # Reset for next measurement: user can click again to start a new ruler
        self._ruler_ptA = None
        self._ruler_ax = None
